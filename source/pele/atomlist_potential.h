...
public:
    ...
    virtual inline double get_energy(Array<double> const & x, const bool soa=false)
    {
        double e=0.;
        size_t jstart = 0;
        double dr[_ndim];
    ...
    virtual inline double add_energy_gradient(Array<double> const & x, Array<double> & grad, const bool soa=false)
    {
        if (x.size() != grad.size()) {
