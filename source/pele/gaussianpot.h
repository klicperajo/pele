...
public:
    ...
    virtual double get_energy_gradient_hessian(Array<double> const & x, Array<double> & grad, Array<double> & hess, const bool soa=false)
    {
        double xTAx = 0;
        for (size_t i = 0; i < m_bdim; ++i) {
            const double tmp = x[i] - m_mean[i];
    ...
    virtual double add_energy_gradient_hessian(Array<double> const & x, Array<double> & grad, Array<double> & hess, const bool soa=false)
    {
        Array<double> grad_term(grad.size());
