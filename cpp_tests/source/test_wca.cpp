...
class SoA_HSWCA_Test : public ::testing::Test {
public:
    size_t seed;
...
TEST_F(SoA_HSWCA_Test, EnergyLeesEdwards_Works) {
    for(double shear = 0.0; shear <= 0.2; shear += 0.1) {
        create_coords();
        pele::HS_WCALeesEdwards<2> pot(eps, sca, radii, boxvec, shear);
...
TEST_F(SoA_HSWCA_Test, EnergyGradientLeesEdwards_Works) {
    for(double shear = 0.0; shear <= 0.2; shear += 0.1) {
        create_coords();
        pele::HS_WCALeesEdwards<2> pot(eps, sca, radii, boxvec, shear);
        for(size_t new_coords = 0; new_coords < 10; new_coords++) {
...
TEST_F(SoA_HSWCA_Test, EnergyGradientHessianLeesEdwards_Works) {
    for(double shear = 0.0; shear <= 0.2; shear += 0.1) {
        create_coords();
        pele::HS_WCALeesEdwards<2> pot(eps, sca, radii, boxvec, shear);
        for(size_t new_coords = 0; new_coords < 10; new_coords++) {
